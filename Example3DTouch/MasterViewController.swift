...
//  Example3DTouch
//
//  Created by Amit kumar Swami on 18/02/16.
...
extension MasterViewController: UIViewControllerPreviewingDelegate {
    ...
    func previewingContext(previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        if let indexPath = tableView.indexPathForRowAtPoint(location) {
            let viewRectInTableView = tableView.rectForRowAtIndexPath(indexPath)
    ...
    func viewControllerForIndexPath(indexPath: NSIndexPath) -> UIViewController {
        let object = objects[indexPath.row] as! NSDate
        let controller = UIStoryboard(name: "Main", bundle: NSBundle.mainBundle()).instantiateViewControllerWithIdentifier("DetailViewController") as! DetailViewController
